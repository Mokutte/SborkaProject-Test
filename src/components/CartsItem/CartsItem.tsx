...
import { SneakerState } from "../../store/types";
import { addInfo, addSneakers } from "../../store/sneakersSlice";
import { useAppDispatch } from "../../hooks/redux";
import { useNavigate } from "react-router-dom";

export function CartsItem(props: SneakerState) {
  const dispatch = useAppDispatch();

  const navigate = useNavigate()

  const goSneakInfo = () => navigate("/sneakerInfo")
  ...
  return (
    <div className={s.sneakerCart} onClick={sendInfo}>
      <div className={s.imgsneakerBlock}>
        <img
          className={s.imgsneaker}
          src={(`/media/images/${props.img}HD.png`)}
          alt="Изображение кросовка"
        />
      </div>
      <div className={s.buyBlock}>
        <p className={s.sneakerName}>{props.name}</p>
        <div className={s.priceBlock}>
          ...
          <p className={s.price}>$ {props.price.toLocaleString("ru")}</p>
        </div>
      </div>
    </div>
  );
