...
import { addSneakers } from "../../store/sneakersSlice";
import { useAppDispatch, useAppSelector } from "../../hooks/redux";
import s from "./style.module.css";
import { BasketState } from "../../store/types";

export function SneakerInfo() {
  const { sneakerInfo } = useAppSelector((state) => state.sneakersReducer);
  const navigate = useNavigate();
  const dispatch = useAppDispatch();

  React.useEffect(() => {
    const json = JSON.stringify(sneakerInfo);
    localStorage.setItem("sneakerInfo", json);
  }, []);

  const goSneakInfo = () => navigate(-1);

  const addItem = (e: any) => {
    e.stopPropagation();
    const item: BasketState = {
      id: sneakerInfo.id || 0,
      name: sneakerInfo.name || "",
      img: sneakerInfo.img || "",
      price: sneakerInfo.price || 0,
    };
  ...
  return (
    <section className={s.mainBlock}>
      <div className={s.firstBlock}>
        <button className={s.backButton} onClick={goSneakInfo}>
          Back in catalog
        </button>
        <img
          className={s.sneakerImg}
          src={`/media/images/${sneakerInfo.img}FHD.png`}
          alt="Изображение кросовка"
        />
      </div>
      <div>
        <p className={s.sneakerName}>{sneakerInfo.name}</p>
        <span className={s.idNumber}>Item model number: {sneakerInfo.id}</span>
        <div className={s.priceBlock}>
          <button className={s.buy} onClick={addItem}>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <p className={s.price}>$ {sneakerInfo.price}</p>
        </div>
      </div>
    </section>
  );
